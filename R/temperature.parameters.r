...
temperature.parameters = function( p=NULL, current.year=NULL, DS="default" ) {
  ...
  if (DS=="spacetime") {
    ...
    if (!exists("spacetime_engine", p)) {
      message( "'spacetime_engine' was not specified, using gam as default")
      p$spacetime_engine = "gam" 
    }

    if (p$spacetime_engine == "spate" ){

    } else if (p$spacetime_engine == "gaussianprocess2Dt") {
