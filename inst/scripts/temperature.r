  ...
  if (create.interpolated.results.lstfilter ) {

    p$lstfilter_local_modelengine="spate"
    ...
    # 2. lstfilter interpolations assuming some seasonal pattern
    ...
    p = lstfilter( DATA='hydro.db( p=p, DS="lstfilter.input" )', p=p, storage.backend="bigmemory.ram" )

    # 3. simple spatial interpolation .. collect data from lstfilter and break into sub-areas defined by p$subregions = c("canada.east", "SSE", "SSE.mpa", "snowcrab" ) .. "regridding"
    ...
    p = make.list( list( yrs=p$tyears), Y=p )
    parallel.run( temperature.db, p=p, DS="lstfilter.prediction.redo" )
    #  temperature.db( p=p, DS="lstfilter.prediction.redo" ) # 2hr in serial mode
